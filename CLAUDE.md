...
- **Device selection:** capture is WASAPI throughout. Mic-only uses sounddevice's WASAPI input; loopback and "Both" use PyAudioWPatch. Config stores device *names* (`mic_device_name`/`loopback_device_name`), not indices — the recorder resolves a name to a concrete device (and its native sample rate) at open time, tolerant of small library-to-library name differences, falling back to the system default. Empty name = default. Legacy MME devices are intentionally excluded (they can open but deliver nothing).
